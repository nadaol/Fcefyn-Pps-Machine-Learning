...
def map_captions(caption_seq,img_name,caption_id):

    tokenizer = load_tokenizer()
    caption_id = '%012d' % (caption_id)
    img_id = img_name.split('_')[2][:12]
    enc_cap_path = encoded_captions_path + 'encodedText_' + img_id + '_' + caption_id + '.emdt'
    cap_tensor = np.load(enc_cap_path,allow_pickle=True)
    caption_string = cap_seq_to_string(caption_seq,tokenizer)

    return cap_tensor,caption_string,img_id

def get_caption_tensor(cption_id):
    enc_cap_path = encoded_captions_path + 'encodedText_' + img_id + '_' + caption_id + '.emdt'
    cap_tensor = np.load(enc_cap_path,allow_pickle=True)
    return cap_tensor

def get_enc_caption_caption_image_id_list(limit):

    images_names_train, images_names_eval , captions_train, captions_eval , tokenizer = get_image_names_captions_lists()

    captions_list = []
    caption_id = 1
    for caption_seq,img_name in zip(captions_eval,images_names_eval):
       captions_list.append(map_captions(caption_seq,img_name,caption_id))
       caption_id+=1
       if(caption_id == limit):
...
def write_embeddings(caption_list,file_name,metadata1_title,metadata2_title,tensors_save_path,metadata_save_path):
    ...
    for tensor,metadata1,metadata2 in caption_list:
...
caption_list = get_enc_caption_caption_image_id_list(100)
print('Caption embedding : %s \nCaption String %s \n Correlated Image Id %s\n' % (caption_list[0][0],caption_list[0][1],caption_list[0][2]) )

write_embeddings(caption_list,'Captions','Caption','Image_Id',enc_caption_tsv,caption_metadata_tsv)